...
@php($gozleyenAvatar = \App\Models\InvestigatorProfile::query()->where('avatar_status', 'pending')->count())
...
<a href="{{ route('admin.avatars') }}" @if(request()->routeIs('admin.avatars*')) aria-current="page" @endif>
  Avatar növbəsi
  @if($gozleyenAvatar > 0)<span class="count badge-open">{{ $gozleyenAvatar }}</span>@endif
</a>
