...
(function (root) {
  ...
  function suNisani(C) {
    var ctx = C.ctx, u = C.u;
    ctx.save();
    /* Seyrək və zəif: alıcı ödənişdən əvvəl dizaynı GÖRƏ bilməlidir.
       Toy dəvətnaməsində «bəyənmədim» riski yüksəkdir — su nişanı oğurluğu
       çətinləşdirməlidir, nəticəni gizlətməməlidir. */
    ctx.globalAlpha = C.P.qara ? 0.13 : 0.10;
    ctx.fillStyle = C.P.murekkeb;
    ctx.font = font(34 * u, 700, F.sans);
    ctx.textAlign = 'center';
    ctx.textBaseline = 'middle';
    ctx.translate(C.W / 2, C.H / 2);
    ctx.rotate(-Math.PI / 5);
    var step = 330 * u, n = Math.ceil(Math.max(C.W, C.H) / step) + 1;
    for (var r = -n; r <= n; r++) {
      for (var c = -n; c <= n; c++) {
        cizSetir(ctx, 'NÜMUNƏ', c * step * 1.05, r * step, 9 * u, 'center');
      }
    }
    ctx.restore();
  }
  ...
  function draw(ctx, inv, opts) {
    ...
    var S = mezmun(inv);
    ctx.save();
    /* `scale` — canlı önizləmə ekran ölçüsündə çəkilsin deyə. Bütün daxili
       hesablamalar məntiqi ölçüdə (r.w × r.h) qalır, yəni önizləmə ilə
       ixrac olunan fayl eyni yerləşdirmədir. */
